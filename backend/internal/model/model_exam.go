...
type Exam struct {
	...
	CurrentVersionID uint       `gorm:"index;not null;default:0" json:"current_version_id"`
	Revision         uint       `gorm:"not null;default:0" json:"-"`
	CreatedBy        uint       `gorm:"index" json:"created_by"`
	CreatedAt        time.Time  `json:"created_at"`
	UpdatedAt        time.Time  `json:"updated_at"`
}

// ExamQuestion is a question selected into the mutable draft paper (working copy).
// Once an exam is published, attempts and reviews never read this table directly;
// they read the frozen PaperVersion snapshot instead.
